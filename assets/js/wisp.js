...
(function () {
  ...
  // The bundled demo only ever stands in when NO backend is configured. A
  // configured site that cannot reach its server must say so, not quietly
  // show fictional works by fictional authors: those "randomly appearing"
  // stories cannot be blocked or muted because they are not real.
  function demoAllowed() { return !(window.WispDB && WispDB.configured); }
  function activeWorks() { return isLive() ? LIVE.works : (demoAllowed() ? W.WORKS : []); }
  function activeById(id) { return LIVE.byId[id] || (demoAllowed() ? W.byId[id] : null); }
  // The reader's own mutes and blocks, applied to any list of works. Every
  // surface that shows works runs through this: home, browse, staff picks,
  // the resume card, tag and hub pages.
  function applyReaderFilters(list) {
    let out = list || [];
    if (userState.mutedTags.size) out = out.filter(w => w && !userState.mutedTags.has(w.source) && !(w.tags || []).some(t => userState.mutedTags.has(t)));
    if (userState.blockedUsers.size) out = out.filter(w => w && !userState.blockedUsers.has(w.author));
    return out;
  }
  ...
  function renderHomeLive() {
    ...
    const works = applyReaderFilters(following ? (LIVE.followingWorks || []) : (LIVE.works || []));
    if (LIVE.featured) LIVE.featured = applyReaderFilters(LIVE.featured);
    if (LIVE.resume && LIVE.resume.work && !applyReaderFilters([LIVE.resume.work]).length) LIVE.resume = null;
    ...
    $("#screen-reading").innerHTML = `
      <div class="reader">
        ...
        <div class="hl-pop" id="hlPop">
          ...
          <button data-hl="un" hidden>Remove highlight</button>
    ...
    $("#screen-reading").innerHTML = `
      <div class="reader${isComic ? " reader--comic" : ""}">
        ...
        <div class="hl-pop" id="hlPopLive">
          ...
          <button data-lhl="un" hidden>Remove highlight</button>
  ...
  function safeHighlightRange(range) {
    ...
    const wrapOne = (r) => {
      ...
      if (frag.querySelector && frag.querySelector(UNSAFE)) return;
      if (!(frag.textContent || "").trim()) return;   // nothing but whitespace: no sliver
    ...
    try {
      ...
      (slices.length ? slices : [range]).forEach(r => { try { wrapOne(r); } catch (e) {} });
    } catch (e) {}
    return made;
  }
  ...
  function removeHighlightMarks(marks) {
    const ids = new Set();
    marks.forEach(m => {
      if (m.dataset && m.dataset.hlId) ids.add(m.dataset.hlId);
  ...
  function wireLiveHighlights(w, ch) {
    ...
    if (!pop || !prose) return;
    let lastText = "", lastRange = null, lastMarks = [];
    ...
    prose.addEventListener("mouseup", (e) => {
      ...
      const range = selected ? sel.getRangeAt(0) : null;
      // A tap on an existing highlight (no selection) offers to remove it.
      lastMarks = highlightMarksAt(prose, e.target, range);
      if (!selected && !lastMarks.length) { hide(); return; }
      lastText = selected ? sel.toString().trim() : lastMarks.map(m => m.textContent).join(" ");
      lastRange = range;
      pop.querySelector('[data-lhl="mark"]').hidden = !selected;
      pop.querySelector('[data-lhl="note"]').hidden = !selected;
      pop.querySelector('[data-lhl="un"]').hidden = !lastMarks.length;
      const rect = (range || (() => { const r = document.createRange(); r.selectNode(lastMarks[0]); return r; })()).getBoundingClientRect();
    ...
    pop.querySelectorAll("[data-lhl]").forEach(b => b.addEventListener("click", async () => {
      ...
      if (kind === "un") {
        ...
        if (ids.length && WispDB.signedIn && WispDB.deleteHighlight) {
          Promise.all(ids.map(id => WispDB.deleteHighlight(id).catch(() => {})))
            .then(() => toast("Highlight removed, here and from your Things."));
        } else {
          toast(ids.length ? "Highlight removed." : "Highlight removed here. Its saved copy stays in Library, under Things.");
        }
        return;
      }
      if (!WispDB.signedIn) { openAuth("in"); hide(); return; }
      if (!text) { hide(); return; }
      const made = safeHighlightRange(lastRange);
      hide(); window.getSelection().removeAllRanges();
      if (kind === "note") { openNoteDialog(w, ch, text); return; }
      try {
        const row = await WispDB.saveHighlight({ work_id: w.id, chapter_id: ch.id, text });
        // Remember which saved row these marks belong to, so removing the
        // highlight later also deletes it from Things.
        if (row && row.id) made.forEach(m => { m.dataset.hlId = row.id; });
        toast("Highlighted. Find it in Library, under Things.");
      }
  ...
  function chapterWordCount() {
    ...
    if (prose !== proseWordsEl) {
      proseWordsEl = prose;
      proseWords = countWordsIn(prose, "figure, iframe, .thread-slot, .para__marker, .embed, .embed-block");
    }
    return proseWords;
  }
  ...
  function wireReading() {
    ...
    // Highlight popover on text selection; a tap on an existing highlight
    // offers to remove it, right here in the reader.
    const pop = $("#hlPop");
    const proseEl = $("#prose");
    let lastRange = null, lastMarks = [], lastText = "";
    proseEl.addEventListener("mouseup", (e) => {
      const sel = window.getSelection();
      const selected = sel && !sel.isCollapsed && sel.toString().trim();
      const range = selected ? sel.getRangeAt(0) : null;
      lastMarks = highlightMarksAt(proseEl, e.target, range);
      if (!selected && !lastMarks.length) { pop.style.display = "none"; return; }
      lastRange = range;
      lastText = selected ? sel.toString().trim() : lastMarks.map(m => m.textContent).join(" ");
      pop.querySelector('[data-hl="mark"]').hidden = !selected;
      pop.querySelector('[data-hl="note"]').hidden = !selected;
      pop.querySelector('[data-hl="un"]').hidden = !lastMarks.length;
      const rect = (range || (() => { const r = document.createRange(); r.selectNode(lastMarks[0]); return r; })()).getBoundingClientRect();
      const host = $("#screen-reading").getBoundingClientRect();
      pop.style.display = "flex";
      pop.style.left = Math.max(8, rect.left - host.left + rect.width / 2 - 70) + "px";
    ...
    pop.querySelectorAll("[data-hl]").forEach(b => b.addEventListener("click", () => {
      ...
      if (kind === "copy") { navigator.clipboard && navigator.clipboard.writeText(lastText || String(window.getSelection())); toast("Copied."); }
      else if (kind === "un") { removeHighlightMarks(lastMarks); lastMarks = []; toast("Highlight removed."); }
      else if (lastRange) {
  ...
  // Count the words in an element, block-aware: element boundaries count as
  // spaces, so the last word of one paragraph never fuses with the first word
  // of the next (plain textContent concatenates them and undercounts).
  function countWordsIn(root, stripSel) {
    const clone = root.cloneNode(true);
    if (stripSel) clone.querySelectorAll(stripSel).forEach(x => x.remove());
    const parts = [];
    (function walk(n) {
  ...
  function updateWordCount() {
    ...
    const words = countWordsIn(ed, "figure, iframe");
  ...
  async function loadHubPage(id) {
    ...
    try {
      ...
      const works = applyReaderFilters(await WispDB.worksInHub(detail.hub).catch(() => []));
  ...
  function renderTagPage() {
    ...
    const works = applyReaderFilters(tp.works || []);
  ...
  // Only the station's own member may upload it. The guard runs again when
  // the debounce fires, because on a phone the sign-in sync can still be in
  // flight when a widget is tapped: without the re-check, the previous
  // member's sticky notes upload into the new member's account, and from
  // then on every device faithfully syncs the leak.
  function mwMayPush() {
    if (!(window.WispDB && WispDB.enabled && WispDB.signedIn && WispDB.saveWidgets && WispDB.profile)) return false;
    let owner = null; try { owner = localStorage.getItem("wisp.widgets.owner"); } catch (e) {}
    return owner === WispDB.profile.id;
  }
  ...
  function mwSyncFromAccount() {
    ...
    // An account that has EVER saved a station is authoritative, even when the
    // saved station is empty: widgets removed on the laptop must not be
    // resurrected by a phone's stale local copy. Only an account with no
    // saved blob at all can be seeded from the device.
    const hasBlob = !!(remote && typeof remote === "object");
    const hasWidgetKeys = hasBlob && (("installed" in remote) || ("state" in remote));
    const hasCompanion = hasBlob && remote.companion && typeof remote.companion === "object";
    if (hasWidgetKeys) {
    ...
    // First sign-in from this device, account never saved a station: keep
    // what was built here (a guest station survives creating the account).
    if (!hasWidgetKeys && !hasCompanion && !foreign) mwPersistRemote();
  }
  // Signing out takes the station off the screen: sticky notes are someone's
  // notes, and whoever is at this device next starts from the shelf, not from
  // them. The owner marker stays, so the same member signing back in is not a
  // stranger, and the pull brings their station straight back.
  function mwHandleSignOut() {
    if (mwRemoteTimer) { clearTimeout(mwRemoteTimer); mwRemoteTimer = null; }   // a push scheduled while signed in dies with the session
    MW.installed = []; MW.state = {};
    mwPersistLocal();
    mwSyncedFor = null;
  ...
  function route() {
    ...
    const live = isLive();
    if (!live && !demoAllowed()) { renderOffline(screen); return; }
    if (screen === "reading") {
  ...
  // A configured site that could not reach its server: say so and offer a
  // retry, instead of quietly showing the bundled demo stories.
  function renderOffline(screen) {
    const el = $("#screen-" + (screen || "home")); if (!el) return;
    el.innerHTML = `
      <div class="page">
        <div class="editorial" style="text-align:center;padding:70px 20px">
          <p class="soft" style="font-size:17px;margin-bottom:8px">Wisp can't reach its server right now.</p>
          <p class="muted" style="font-size:13.5px;margin-bottom:18px">Check your connection, then try again.</p>
  ...
  const GRAMMAR_RULES = [
    ...
    // ---- Real-word slips: spelled right, still the wrong word --------------
    // "a new king of magic" is nearly always "kind of"; but kings rule fantasy
    // prose, so only clearly non-royal frames trip (never "the king of Gondor").
    { re: /\b(a|another) (new|different|whole new|special|certain|strange|rare|weird|odd) king(s)? of\b/gi,
      fix: (m) => m[1] + " " + m[2] + " kind" + (m[3] || "") + " of" },
    { re: /\b(some|what|whatever) king of\b/gi, fix: (m) => m[1] + " kind of" },
    gcPhrase("the hole time", "the whole time"), gcPhrase("the hole thing", "the whole thing"),
    gcPhrase("the hole point", "the whole point"), gcPhrase("the hole story", "the whole story"),
    gcPhrase("a hole new", "a whole new"),
    gcPhrase("weather or not", "whether or not"),
    gcPhrase("accept for", "except for"),
    gcPhrase("a part from", "apart from"), gcPhrase("apart of", "a part of"),
    { re: /\b(was|were|is|am|are|been|being) suppose to\b/gi, fix: (m) => m[1] + " supposed to" },
    { re: /\b(I|we|they|he|she|you) use to\b/gi, fix: (m) => m[1] + " used to" },
    { re: /\b(I|he|she|it|we|they|you) where\b/gi, fix: (m) => m[1] + " were" },
    { re: /\bto by (a|an|the|some|it|them|one|more|new|another)\b/gi, fix: (m) => "to buy " + m[1] },
    { re: /\b(I|we|they|you) no (that|what|how|why|where|it|him|her|if|about)\b/gi, fix: (m) => m[1] + " know " + m[2] },
    { re: /\b(don't|doesn't|didn't|won't|wouldn't|can't) no\b/gi, fix: (m) => m[1] + " know" },
    { re: /\b(I|we|they|he|she|you) new (that|what|how|about|it|him|her|them)\b/gi, fix: (m) => m[1] + " knew " + m[2] },
    gcPhrase("quite down", "quiet down"), gcPhrase("peace and quite", "peace and quiet"),
    gcPhrase("quite as a mouse", "quiet as a mouse"),
    { re: /\b(to|will|would|might|could|gonna|don't|didn't|not) loose\b/gi, fix: (m) => m[1] + " lose" },
    { re: /\b(to|can't|couldn't|barely|hardly) breath\b/gi, fix: (m) => m[1] + " breathe" },
    { re: /\b(came|come|comes|coming|away|different|suffer|suffers|suffering|made|hear|heard|far) form\b/gi, fix: (m) => m[1] + " from" },
    { re: /\bit's (own|way|place|head|eyes|hands|name|tail|wings|feet|back|side|heart|mouth|claws|paws)\b/gi, fix: (m) => "its " + m[1] },
    { re: /\byou're (own|name|hand|hands|head|eyes|face|life|heart|way|family|friend|friends|mother|father|brother|sister|book|story|work|room|fault|majesty|highness)\b/gi, fix: (m) => "your " + m[1] },
    { re: /\bthey're (own|way|hands|eyes|heads|lives|hearts|faces|backs)\b/gi, fix: (m) => "their " + m[1] },
    { re: /\b(their|they're) (is|are|was|were)\b/gi, fix: (m) => "there " + m[2] },
    gcPhrase("an then", "and then"),
    { re: /\b(walked|ran|drove|flew|went|rushed|moved|brushed|pushed|slipped|stormed) passed\b/gi, fix: (m) => m[1] + " past" },
    { re: /\b(has|have|had|she|he|they|we|I|you) past (away|out)\b/gi, fix: (m) => m[1] + " passed " + m[2] },
    { re: /\b(not|isn't|aren't|wasn't|weren't|be|been|being) aloud\b/gi, fix: (m) => m[1] + " allowed" },
    gcPhrase("bare with me", "bear with me"),
    { re: /\b(can't|couldn't|cannot) bare\b/gi, fix: (m) => m[1] + " bear" },
    { re: /\b(too|so|very|felt|feel|feels|feeling|grew|growing) week\b/gi, fix: (m) => m[1] + " weak" },
    { re: /\b(can|can't|could|couldn't|didn't|don't|won't|to) here (you|me|him|her|them|us|it|the|a|an|what|anything|something|nothing)\b/gi, fix: (m) => m[1] + " hear " + m[2] },
    gcPhrase("a peace of", "a piece of"),
    { re: /\b(web|camp|construction) sight\b/gi, fix: (m) => m[1] + " site" },
  ...
  // and an all-caps original (a shouted line, a title) keeps its caps.
  function gcApplyCase(original, fix) {
    if (original && original.length > 1 && original === original.toUpperCase() && /[A-Z]/.test(original))
  ...
  // Words may carry straight OR curly apostrophes: phones type didn’t with a
  // smart quote, and treating ’ as a word break split it into "didn" + "t",
  // which then drew a red squiggle with useless suggestions.
  const GC_WORD_RE = /[A-Za-z][A-Za-z'’]*/g;
  // The dictionary speaks straight apostrophes; check and suggest with this.
  function gcNorm(w) { return w.replace(/’/g, "'"); }
  ...
  function gcBackend() {
    ...
    gcBackendP = (async () => {
      ...
      try {
        ...
        const glue = "\n;var __dict=null;" +
          // The obvious fix first: a missing apostrophe (didnt, youre, wasnt)
          // repairs by trying ' at every position, far better than edit-distance.
          "function __apos(w){var out=[];if(!__dict||w.indexOf(\"'\")>=0)return out;for(var i=1;i<w.length;i++){var c=w.slice(0,i)+\"'\"+w.slice(i);if(__dict.check(c))out.push(c);}return out;}" +
          "self.onmessage=function(ev){var m=ev.data;try{" +
          ...
          "else if(m.t==='suggest'){var s=__apos(m.word);var h=__dict?(__dict.suggest(m.word,6)||[]):[];for(var j=0;j<h.length;j++){if(s.indexOf(h[j])<0)s.push(h[j]);}self.postMessage({t:'suggest',word:m.word,sugg:s});}" +
      ...
      try {
        ...
        const apos = (w) => {
          ...
          for (let i = 1; i < w.length; i++) { const c = w.slice(0, i) + "'" + w.slice(i); if (dict.check(c)) out.push(c); }
          return out;
        };
        return {
          ...
          suggest(word) {
            const s = apos(word);
            (dict.suggest(word, 6) || []).forEach(h => { if (s.indexOf(h) < 0) s.push(h); });
  ...
  function gcSpellCandidates(text, out) {
    ...
    while ((m = GC_WORD_RE.exec(text))) {
      ...
      // A trailing apostrophe is punctuation (a quote, a possessive plural),
      // not part of the word.
      while (w.length && (w.endsWith("'") || w.endsWith("’"))) { w = w.slice(0, -1); e--; }
      ...
      const norm = gcNorm(w), lower = norm.toLowerCase();
      const known = (x) => gcState.userDict.has(x) || gcState.ignoredWords.has(x);
      if (known(lower)) continue;
      // "Orcus's" and "Orcuses" stay quiet once "Orcus" is in the dictionary:
      // possessives and plain plurals inherit their base word's standing.
      const poss = lower.replace(/'s$/, ""), plur = lower.replace(/e?s$/, "");
      if ((poss !== lower && known(poss)) || (plur !== lower && known(plur))) continue;
      out.push({ s: s, e: e, orig: w, norm: norm });
    }
  }
  function gcRuleIssues(text) {
    ...
    GRAMMAR_RULES.forEach(rule => {
      ...
      while ((m = rule.re.exec(text))) {
        ...
        let fix = typeof rule.fix === "function" ? rule.fix(m) : (rule.caps ? gcApplyCase(m[0], rule.fix) : rule.fix);
        // A shouted line or a title stays shouted: KING OF -> KIND OF.
        if (fix != null && m[0].length > 1 && m[0] === m[0].toUpperCase() && /[A-Z]/.test(m[0])) fix = fix.toUpperCase();
  ...
  async function gcScan() {
    ...
    try {
      ...
      todo.forEach(x => (x.c.words || []).forEach(w => wordSet.add(w.norm)));
      let bad = new Set();
      ...
      let repaint = false;
      todo.forEach(x => {
        const spell = (x.c.words || []).filter(w => bad.has(w.norm)).map(w => ({ s: w.s, e: w.e, orig: w.orig, norm: w.norm, kind: "spell" }));
        x.c.spellDone = true;
  ...
  function gcQueueSuggestions() {
    ...
    gcBlocksOf(ed).forEach(block => {
      ...
      if (c) c.issues.forEach(i => {
        const key = (i.norm || i.orig).toLowerCase();
        if (i.kind === "spell" && !gcState.suggCache.has(key) && !seen.has(key)) {
          seen.add(key); q.push(i.norm || i.orig);
        }
      });
    });
  ...
  async function gcPumpSuggestions() {
    ...
    try {
      ...
      while (gcState.suggQueue.length) {
        ...
        const near = sugg.filter(x => x && x.toLowerCase() !== key &&
          (x.replace(/'/g, "") === word.replace(/'/g, "") || levDist(word, x) <= 2)).slice(0, 4);
  ...
  function gcAltsFor(issue) {
    ...
    const near = gcState.suggCache.get((issue.norm || issue.orig).toLowerCase());
    if (near == null) return null;   // still thinking
    // Suggestions wear the writer's own apostrophe style: a phone's curly
    // quote stays curly when the fix goes in.
    const styled = /’/.test(issue.orig) ? near.map(x => x.replace(/'/g, "\u2019")) : near;
    return styled.map(x => matchCase(issue.orig, x));
  }
  function gcCardHTML(issue) {
    ...
    return `
      ...
      <div class="gc-card__foot">
        ...
        <button class="gc-card__link" data-gc-ignore-all>Ignore all</button>
        ${spell ? `<button class="gc-card__link" data-gc-add>Add to dictionary</button>` : ""}
      </div>`;
  }
  function gcCardRefresh(word) {
    if (!gcCard || !gcCardCtx || (gcCardCtx.issue.norm || gcCardCtx.issue.orig).toLowerCase() !== word.toLowerCase()) return;
  ...
  function gcCardWire() {
    ...
    // Ignore dismisses exactly THIS underline; the rest of the page keeps its
    // squiggles. (It returns if the paragraph is edited, which is the honest
    // behavior: the text changed, so it gets re-checked.)
    const ig = gcCard.querySelector("[data-gc-ignore]");
    if (ig) ig.addEventListener("click", () => {
      const c = gcState.cache.get(block);
      if (c) c.issues = c.issues.filter(i => i !== issue);
      closeGrammarPop(); gcRender();
    });
    // Ignore all quiets this exact word (or phrase) everywhere, and nothing else.
    const igAll = gcCard.querySelector("[data-gc-ignore-all]");
    if (igAll) igAll.addEventListener("click", () => {
      if (issue.kind === "spell") gcState.ignoredWords.add((issue.norm || issue.orig).toLowerCase());
    ...
    if (add) add.addEventListener("click", () => {
      gcState.userDict.add((issue.norm || issue.orig).toLowerCase()); gcSaveUserDict();
  ...
  function openGrammarPop(hit) {
    ...
    if (hit.issue.kind === "spell" && !gcState.suggCache.has((hit.issue.norm || hit.issue.orig).toLowerCase())) {
      const w0 = hit.issue.norm || hit.issue.orig;
      gcState.suggQueue = [w0].concat(gcState.suggQueue.filter(w => w !== w0));
      gcPumpSuggestions();
    }
  }
  ...
  function currentEditorFontSize() {
    ...
    // A selection that already holds a sized span steps from that span's size,
    // so nudging a resized run keeps climbing instead of resetting to the body.
    if (!sel.isCollapsed && sel.rangeCount) {
      try {
        const frag = sel.getRangeAt(0).cloneContents();
        const sp = frag.querySelector && frag.querySelector("span[style*='font-size']");
        if (sp) { const v = Math.round(parseFloat(sp.style.fontSize)); if (v) return v; }
      } catch (e) {}
    }
  ...
  function applyFontSize(px) {
    ...
    const range = sel.getRangeAt(0);
    // Resize one slice at a time, paragraph by paragraph, so a selection that
    ...
    catch (e) { toast("That selection could not be resized."); return; }
    ...
    if (live.length) {
      try {
        ...
        r2.setStartBefore(live[0]); r2.setEndAfter(live[live.length - 1]);
        sel.removeAllRanges(); sel.addRange(r2); rememberEditorSelection();
      } catch (e) {}
    }
  ...
  function boot() {
    ...
    const doSignInOut = () => {
      ...
      if (WispDB.signedIn) WispDB.signOut().then(() => { guestBrowsing = false; mwHandleSignOut(); toast("Signed out."); updateAuthGate(); });
      else openAuth("in");
    };
    ...
    // Ctrl/Cmd+K makes a link from the selection, the shortcut every editor
    // shares.
    document.addEventListener("keydown", (e) => {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (!nav) return;
      // The nav's edges are Previous and Next when they exist (a spacer span
      // stands in otherwise); only real chapter links respond.
      const edge = e.key === "ArrowRight" ? nav.lastElementChild : nav.firstElementChild;
      if (edge && edge.matches && edge.matches("button[data-read]")) { e.preventDefault(); edge.click(); }
    });
